		...
		if ( function_exists( 'fw_get_db_settings_option' )  ) {
			...
			if ( get_post_type() == 'page' ) {
				...
				if ( !empty($current_background) ) {
					...
					wp_add_inline_style( 'coffeeking-theme-style', '.page-header { background-image: url(' . esc_url( $current_background ) . ') !important; } ' );
				}
			}
			...
			if (! empty( $logo_height ) ) {
				...
				wp_add_inline_style( 'coffeeking-theme-style', 'nav.navbar .logo img { max-height: ' . esc_attr( $logo_height ) . 'px !important; } ' );
			}
